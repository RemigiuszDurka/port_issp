...
Ciąg to funkcja określona na zbiorze liczb naturalnych $\mathbb{N}$, oznaczana jako $\{a_n\}_{n=1}^{\infty}$. Jej wyrazy mogą być liczbami rzeczywistymi, zespolonymi lub innymi obiektami matematycznymi. Szereg to suma wyrazów ciągu, np. $\displaystyle \sum_{n=1}^{N} a_n$. Gdy $N$ dąży do nieskończoności, mówimy o szeregu nieskończonym.
...
1. Jaki to ciąg: $4,4,4,4,4,4,\ldots$?
...
5. Podaj wzór rekurencyjny $a_{n+1}=a_n+\ldots$ ciągu arytmetycznego, którego pierwszy wyraz ma wartość 5, a dziesiąty wyraz ma wartość 23.
...
12. Udowodnij, że w ciągu arytmetycznym każdy wyraz $a_n$ jest średnią arytmetyczną swoich sąsiadów:

    $$
    a_n=\frac{a_{n-1}+a_{n+1}}{2}.
    $$

13. Udowodnij, że w ciągu geometrycznym każdy wyraz $a_n$ jest średnią geometryczną swoich sąsiadów:
...
4. W pewnym eksperymencie dostaliśmy wartość $1.291285\ldots$. Sprawdź na stronie [OEIS](https://oeis.org/), czy za tym nie stoi coś bardziej fundamentalnego. Wskazówka: na stronie wpisz `1,2,9,1,2,8,5`.

5. Mamy ciąg $1,2,4,7,11,16,\ldots$, gdzie widzimy, że przyrosty są zadane kolejnymi liczbami naturalnymi. Używając strony [OEIS](https://oeis.org/), ustal, jaka jest postać $a_n$.

6. Ile wynosi suma szeregu
...
8. Podaj wzór na całkowitą liczbę bloków potrzebnych do stworzenia piramidy o $n$ stopniach. Zacznij od narysowania, jak taka konstrukcja wygląda od góry — jeden blok na warstwie 4 bloków, które z kolei leżą na warstwie 9 bloków, a te na warstwie 16 bloków itd.
...
9. Według pewnej legendy związanej z szachami pewien mędrzec miał poprosić króla o „skromną” nagrodę:
...
10. Mrówka siedząca w początku układu współrzędnych postanowiła pójść 1 metr w prawo, następnie $1/2$ metra w górę, następnie $1/3$ metra w lewo, $1/4$ metra w dół, $1/5$ metra w prawo itd. Do jakiego miejsca ostatecznie doszła? Podaj współrzędne końcowego punktu.

    ![Schemat ruchu mrówki.](../site_assets/mrowka.png){ width="300" }

11. Poniższy program w Octave wyznacza kilka pierwszych elementów [ciągu Viète’a](https://en.wikipedia.org/wiki/Vi%C3%A8te's_formula), który jest zbieżny do $2/\pi$, i na tej podstawie wyświetla kolejne przybliżenia liczby $\pi$:
...
12. Zazwyczaj przy ciągach pada pytanie wyłącznie o finalną granicę, jednak możemy również zapytać, jak osiągana jest ta granica, np. poprzez zachowanie się jak funkcja potęgowa, logarytmiczna, liniowa lub wykładnicza. Na wykładzie omawiany jest przykład ze zbieżnością
...
2. Wyznacz granicę niewłaściwą

   $$
   \lim_{n\to\infty}\frac{n^n}{3^n}.
   $$

3. Ustal, czemu równe są granice
...
6. Czy granica ciągu o elementach wymiernych może być niewymierna?
