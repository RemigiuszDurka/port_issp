...
2. Uporządkuj standardowe zbiory liczbowe przez inkluzję (co w czym się znajduje): $\mathbb N$, $\mathbb Z$, $\mathbb Q$, $\mathbb R$, $\mathbb C$.
   
3. Zamień rozwinięcie okresowe
...
5. Znając już liczby zespolone, znajdź ręcznie miejsca zerowe funkcji kwadratowej $y(x)=x^2+x+1$.
6. Dana jest liczba zespolona $z=1+i$. Zapisz ją w postaci trygonometrycznej.
7. Oblicz ręcznie $z^3$, gdzie $z=1+i$.
8. Dana jest liczba zespolona $z_1=3+4i$ oraz $z_2=1-2i$. Oblicz ręcznie iloraz $z_1/z_2$, podając wynik w postaci $a+ib$.
9. Sprawdź w Octave wartości `sqrt(2)`, `pi`, `eps` oraz `i^2`. Wyjaśnij, które z otrzymanych wartości są przechowywane dokładnie w używanej reprezentacji zmiennoprzecinkowej, a które jedynie w przybliżeniu. Zwróć uwagę, że `eps` nie jest stałą matematyczną taką jak $\pi$: w Octave oznacza odstęp między liczbą 1 a następną większą liczbą reprezentowalną w typie `double`.
10. Dla liczby zespolonej `z = 1 + 1*i` sprawdź w Octave polecenia `abs(z)`, `arg(z)`, `real(z)` i `imag(z)` i powiąż ich wyniki z postacią algebraiczną oraz trygonometryczną liczby zespolonej.


### Logarytmy i potęgi

11. Ile czasu musi upłynąć, by z pewnej masy $m$ izotopu o czasie połowicznego rozpadu 1000 lat pozostał tylko $1\%$ masy początkowej?

12. Zakładając $x>0$ oraz $y>0$, pokaż, że zachodzi
...
13. Stwierdzenie, ile cyfr ma liczba zapisana w postaci $10^{30}$, jest trywialnie proste. Policz ręcznie, ile w przybliżeniu cyfr ma liczba $2^{1000}$, jeśli wiadomo, że $\log_{10}(2)=0.30$. Na pewno przyda Ci się $b=a^{\log_a b}$.

14. Zajrzyj na stronę w angielskiej Wikipedii poświęconą [wykresom półlogarytmicznym](https://en.wikipedia.org/wiki/Semi-log_plot) (ang. *semi-log plots*) a następnie spójrz na poniższe obrazki
...
**Zadanie:** W książce R. Feynmana możemy przeczytać następujący fragment:
